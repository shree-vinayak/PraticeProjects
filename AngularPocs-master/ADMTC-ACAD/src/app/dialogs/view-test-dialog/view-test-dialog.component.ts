...
import { TestCorrectionService } from 'app/services';
import _ from 'lodash';
declare var jsPDF: any;
declare var html2canvas: any;

@Component({
  selector: 'app-view-test-dialog',
  templateUrl: './view-test-dialog.component.html',
  styleUrls: ['./view-test-dialog.component.scss'],
  providers: [DatePipe]
})
export class ViewTestDialogComponent implements OnInit, AfterViewChecked {
  ...
  @ViewChild('PDFPersonalized') documentPagesRefPDFPersonalized: ElementRef;
  ...
  constructor(private testService: TestService,
    ...
    public dialogRef: MdDialogRef<ViewTestDialogComponent>,
  ...
  ngOnInit() {
    ...
    this.userservice.getUserTypesByEntitiesAndStoreThemLocally('academic').subscribe( res => {
      this.userTypes = res;
    });
    ...
    this.appService.getSelectedScholarSeason(this.rncpTitle._id).subscribe((season) => {
      if (season.length !== 0) {
        ...
        console.log(season.scholarseason);
      }
    });
    let dt = new Date();
    let currentYear = dt.getFullYear();
    let nextYear = dt.getFullYear() + 1;
    this.testService.getTest().subscribe(
      function (test) {
        console.log('this.test');
        console.log(this.test);
        //  this.test = test;
        this.docFooterText = 'ADMTC – ' + this.test.name + ' – ' + this.rncpTitle.shortName + ' – ' + currentYear + ' / ' + nextYear;
  ...
  renderData() {
    let self = this;
    let sections = this.test.correctionGrid.correction.sections;
  ...
  getLastPage() {
    let testDetails = `<div style="font-size: 14px; padding: 1rem;">` +
      ...
      `<p>` + this.translate.instant('TEST.DATETYPE') + ` : ` + this.translate.instant('TEST.DATETYPES.' + this.test.dateType.toUpperCase()) + `</p>` +
  ...
  downloadPDF(Status) {
    ...
    if(Status === 'Personalized'){
      ...
      const target = this.documentPagesRefPDFPersonalized.nativeElement.children;
  ...
  printDoc(doc) {
    ...
    doc.text(20, y = y + 24, 'Test ' + this.translate.instant('TEST.TESTDATE') + ' : ' + this.getPrintDate(this.test.date));
  ...
  //  doc.text(20, y = y + 24, this.translate.instant('TEST.ORGANISER') + ' : ' + this.test.organiser);
    ...
    if (this.test.calendar.steps.length > 0) {
      for (let i = 0; i < this.test.calendar.steps.length; i++) {
        ...
        doc.text(20, y = y + 24, (i + 1) + '.  ' + dateString + ' : ' + this.getTranslateWhat(this.test.calendar.steps[i].text));
      }
    } else {
  ...
  getTranslateADMTCSTAFFKEY(userTypeId) {
    // console.log(name);
    let uType = []
    if (this.userTypes && this.userTypes.length > 0) {
      uType = this.userTypes.filter((u) => u._id === userTypeId);
      //   uType = _.filter(this.userTypes, function(u){
      //     return u._id === userTypeId;
      // });
      if (uType[0].name) {
        return this.getUserTypeTranslation(uType[0].name);
      }
    }

  }
  getUserTypeTranslation(name) {
